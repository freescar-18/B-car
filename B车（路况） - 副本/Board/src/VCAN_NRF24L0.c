...
uint8 TX_ADDRESS[5] = {0x00, 0x01, 0x02, 0x03, 0x04};   // 定义一个静态发送地址
uint8 RX_ADDRESS[5] = {0x00, 0x01, 0x02, 0x03, 0x04};
...
{
    //配置NRF管脚复用
    spi_init(NRF_SPI, NRF_CS, MASTER,12500*1000);     // 12500*1000               //初始化SPI,主机模式
    ...
    nrf_writereg(NRF_WRITE_REG + SETUP_RETR, 0x05);                 //设置自动重发间隔时间:250us + 86us;最大自动重发次数:15次
...
{
#define NRF_CHECH_DATA  0x01        //此值为校验数据时使用，可修改为其他值
 ...
 *  @since      v5.0
 */
uint8    nrf_tx(uint8 *txbuf, uint32 len)//txbuf填发送数据的数组名，buff[5]为储存的数组，则写buff
{
